...
const ph = {
  ...
  headerSearchLabel: 'Ask your question',
...
function buildSearch() {
  ...
  const label = el('label', { class: 'nav-search-label' }, ph.headerSearchLabel);
  ...
  const input = el('input', {
    ...
    id: 'nav-search-input',
    name: 'question',
    placeholder: ph.headerSearchLabel,
    autocomplete: 'off',
  });
  ...
  const closeSuggest = wireSuggest(input, suggest, 'nav-search');
...
export default async function decorate(block) {
  ...
  const placeholders = fetchLocalPlaceholders();
